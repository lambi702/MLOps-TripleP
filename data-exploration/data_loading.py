...
def load_data(solar_file_path, weather_file_path):
    ...
    solar_df = pd.read_csv(
        solar_file_path, sep=";", index_col=1, parse_dates=True
    ).sort_index()  #'data/solar.csv'
    ...
    for filename in filenames:
        ...
        df_buf.index = (
            df_buf["YYYY"].astype(int).astype(str)
            + "-"
            + df_buf["MM"].astype(int).astype(str)
            + "-"
            + df_buf["DD"].astype(int).astype(str)
            + " "
            + df_buf["HH"].astype(int).astype(str)
            + ":"
            + df_buf["MIN"].astype(int).astype(str)
        )
        df_buf = df_buf.drop(columns=["YYYY", "MM", "DD", "HH", "MIN"])

        df_buf.index = (
            pd.to_datetime(df_buf.index, format="%Y-%m-%d %H:%M")
            .tz_localize(utc)
            .tz_convert(cest)
        )
    ...
    merged_df = pd.merge(
        solar_df, weather_df, left_index=True, right_index=True, how="outer"
    )
